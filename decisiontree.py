...
def pruneTree(tree, l, k, valData):
    """
...
def varianceImpurity(trainData):
    c = Counter()
    for instance in trainData:
        c[instance.label] += 1
    total = sum(c.values())
...
def informationGain(data, attrNum, parentEntr, entropyI):
    ...
    for attrVal, attrDataList in attrDict.iteritems():
        if entropyI == 'e':
            ...
            infoGain -= float(len(attrDataList))/float(len(data))* varianceImpurity(attrDataList)
...
def main(argv):
    ...
    print "After pruning:"
    prunedTreeIG = pruneTree(decisionTreeIG, l, k, validationData)
    ...
    prunedTreeVI = pruneTree(decisionTreeVI, l, k, validationData)
